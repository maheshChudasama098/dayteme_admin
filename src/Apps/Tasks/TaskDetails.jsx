		...
		<Stack spacing={4}>
			...
			<Stack spacing={2} direction={{xs: "column", md: "row"}} sx={{justifyContent: "space-between", alignItems: {xs: "flex-start", md: "center"}}}>
				...
				<Stack direction="row" spacing={1.5}>
					...
					<Button color="success" variant="contained" startIcon={<Iconify icon="solar:check-circle-bold" />} sx={{borderRadius: 8}} onClick={() => handleAction("Mark as Completed")}>
						Complete Task
					</Button>
					...
					<Menu
						...
						<MenuItem
							...
							<Iconify icon="solar:transfer-horizontal-bold" sx={{mr: 2}} /> Update Status
						</MenuItem>
						<MenuItem onClick={() => handleAction("Assign Task")}>
							<Iconify icon="solar:user-id-bold" sx={{mr: 2}} /> Re-assign Task
						</MenuItem>
						<MenuItem onClick={() => handleAction("Edit Details")}>
							<Iconify icon="solar:pen-bold" sx={{mr: 2}} /> Edit Details
						</MenuItem>
						<MenuItem onClick={() => handleAction("Add Internal Note")}>
							<Iconify icon="solar:document-add-bold" sx={{mr: 2}} /> Add Note
						</MenuItem>
						<MenuItem onClick={() => handleAction("Change Priority")}>
							<Iconify icon="solar:danger-circle-bold" sx={{mr: 2}} /> Change Priority
						</MenuItem>
						<Divider />
						<MenuItem onClick={() => handleAction("Cancel Task")} sx={{color: "error.main"}}>
							<Iconify icon="solar:trash-bin-trash-bold" sx={{mr: 2}} /> Cancel Task
						</MenuItem>
					</Menu>

					<Menu anchorEl={statusAnchorEl} open={Boolean(statusAnchorEl)} onClose={() => setStatusAnchorEl(null)} PaperProps={{sx: {width: 180, borderRadius: 2, mt: 1}}}>
						<MenuItem disabled>
				...
				<Grid size={{xs: 12, md: 8}}>
					<Stack spacing={3}>
						<Card sx={{p: 3, borderRadius: 4, boxShadow: "0 5px 25px rgba(0,0,0,0.05)", border: "none"}}>
								...
								<Box>
									...
									<Stack direction="row" spacing={2} alignItems="center">
										...
										<Stack direction="row" alignItems="center" spacing={0.5}>
											...
											<Typography variant="subtitle2" sx={{color: getPriorityColor(task.priority), fontWeight: 700}}>
							...
							<Grid container spacing={3}>
								...
								<Grid item xs={12} sm={6}>
									...
									<Typography variant="subtitle1" fontWeight="700" sx={{mt: 1, color: "error.main"}}>
										{task.dueDate}
									</Typography>
								</Grid>
							</Grid>
						...
						<NotesManager
							targetId={task.id}
							targetType="Task"
							mockNotes={[{id: "NOTE-1", adminName: "Mark Steave", content: "Awaiting response from venue owner.", date: "Jul 7, 2026, 09:30 AM", isOwn: false}]}
						/>
					</Stack>
				</Grid>
